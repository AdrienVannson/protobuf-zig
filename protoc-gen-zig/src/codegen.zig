...
pub fn generateFile(
    ...
    // Build import table from cross-file type references in this file's messages.
    var imports = ImportTable.init(alloc);
    defer {
        for (imports.values()) |alias| alloc.free(alias);
        imports.deinit();
    }
    try collectImports(alloc, desc_file, &imports);
...
/// Collects all external file dependencies referenced by message fields in cur_file,
/// populating the import table with (DescFile* → alias) entries.
fn collectImports(
    alloc: std.mem.Allocator,
    cur_file: *const protobuf.DescFile,
    table: *ImportTable,
) !void {
    for (cur_file.messages) |*msg| {
        try collectImportsForMessage(alloc, cur_file, msg, table);
    }
}

fn collectImportsForMessage(
    alloc: std.mem.Allocator,
    cur_file: *const protobuf.DescFile,
    msg: *const protobuf.DescMessage,
    table: *ImportTable,
    ...
    for (msg.fields) |*field| {
        const dep_file: ?*const protobuf.DescFile = switch (field.kind) {
            .message_field => |mf| mf.message.file,
            .enum_field => |ef| ef.enum_type.file,
            .list => |list| switch (list.element) {
                .message => |m| m.file,
                .enum_type => |e| e.file,
                .scalar => null,
            },
        ...
        if (dep_file) |df| {
            if (df != cur_file and !table.contains(df)) {
                const alias = try aliasFor(alloc, df);
                errdefer alloc.free(alias);
                try table.put(df, alias);
            }
        }
    }
    for (msg.nested_messages) |*nested| {
        try collectImportsForMessage(alloc, cur_file, nested, table);
    }
}
