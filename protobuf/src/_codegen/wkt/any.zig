...
pub fn pack(comptime AnyT: type, allocator: std.mem.Allocator, msg: anytype) !AnyT {
    const T = @TypeOf(msg);
    const value = try protobuf.to_binary(allocator, msg);
...
pub fn unpack(any: anytype, comptime T: type, allocator: std.mem.Allocator) !T {
    if (!is(any, T)) return error.AnyTypeMismatch;
    var msg: T = .{};
    errdefer msg.deinit(allocator);
    try protobuf.from_binary(&msg, any.value, allocator);
    return msg;
}
