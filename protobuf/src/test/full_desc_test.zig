...
test "_desc resolves within-file message references by pointer identity" {
    ...
    try std.testing.expectEqual(example.Foo._desc(), foo_field.kind.message_field.message);
}

test "_desc links nested message to its parent" {
    const nested = example.Bar.Nested._desc();
    try std.testing.expectEqualStrings("Nested", nested.local_name);
    try std.testing.expectEqual(example.Bar._desc(), nested.parent.?);
}

test "_desc resolves cross-file references" {
    const main = import_main.ImportMain._desc();
