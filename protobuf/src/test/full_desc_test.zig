...
test "_full_desc exposes the linked message descriptor" {
    const foo = example.Foo._full_desc();
...
test "_full_desc resolves within-file message references by pointer identity" {
    const bar = example.Bar._full_desc();
    ...
    // The message_field points at the very same cached DescMessage as Foo._full_desc().
    try std.testing.expectEqual(example.Foo._full_desc(), foo_field.kind.message_field.message);
}

test "_full_desc links nested message to its parent" {
    const nested = example.Bar.Nested._full_desc();
    try std.testing.expectEqualStrings("Nested", nested.local_name);
    try std.testing.expectEqual(example.Bar._full_desc(), nested.parent.?);
}

test "_full_desc resolves cross-file references" {
    const main = import_main.ImportMain._full_desc();
