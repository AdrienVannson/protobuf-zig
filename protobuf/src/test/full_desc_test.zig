...
test "_desc exposes the linked message descriptor" {
    ...
    const foo = example.Foo._desc(io);
...
test "_desc resolves within-file message references by pointer identity" {
    const io = std.testing.io;
    const bar = example.Bar._desc(io);
    ...
    try std.testing.expectEqual(example.Foo._desc(io), foo_field.kind.message_field.message);
}

test "_desc links nested message to its parent" {
    const io = std.testing.io;
    const nested = example.Bar.Nested._desc(io);
    try std.testing.expectEqualStrings("Nested", nested.local_name);
    try std.testing.expectEqual(example.Bar._desc(io), nested.parent.?);
}

test "_desc resolves cross-file references" {
    const io = std.testing.io;
    const main = import_main.ImportMain._desc(io);
